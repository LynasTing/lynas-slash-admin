...
/**
 * 上传区域插画组件，用于在上传入口或空状态中提供和当前主题一致的视觉提示。
 * @returns 上传场景 SVG 插画。
 *
 * Upload illustration component used by upload entry points or empty states to provide a visual cue aligned with the current theme.
 * @returns SVG illustration for upload scenarios.
 */
export default function UploadIllustration() {
  // SVG 主色从主题中读取，确保插画能跟随品牌色或主题切换同步更新。
  // The SVG primary colors come from the theme so the illustration stays aligned with brand and theme changes.
  const {
    themeVars: {
      colors: {
        palette: {
