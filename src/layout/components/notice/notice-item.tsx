...
const FILE_ICON_MAP: Partial<Record<FileType, string>> = {
  ppt: 'local:file-ppt',
  ...
  if (hasAvatar) {
    return (
      <div className="relative">
        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-bg-neutral text-sm font-medium">CH</div>
    ...
    return (
      <div className="mt-3 space-y-2">
        {props.files.map(item => (
          <div key={item.name} className="flex items-center space-x-4 rounded-lg bg-bg-neutral p-3">
...
function NotificationTask(props: Pick<NotificationProps, 'id' | 'task' | 'tags'>) {
  ...
  if (task) {
    return (
      <div className="mt-3 rounded-lg bg-bg-neutral pt-3">
        <div className="flex items-center justify-between">
          <div className="flex-1">
            ...
            <div className="mt-2 flex items-center space-x-2">
              {tags?.map(item => (
                <span key={item} className={`rounded-full px-2 py-1 text-xs ${getTagColor(item)}`}>
                  {item}
                </span>
              ))}
            </div>
          ...
          <div className="flex -space-x-2">
            {task.assignees &&
              Array.from({ length: task.assignees }).map((_, idx) => (
                <div key={`attendee-${id}-${idx}`} className="h-6 w-6 rounded-full border-2 bg-bg-neutral" />
              ))}
          </div>
        </div>
      </div>
...
export default function NoticeItem(notice: NotificationProps) {
  return (
    <div className="flex items-start space-x-3 border-b border-border py-4 last:border-b-0">
